    ...
    <div className="grid grid-cols-2 md:grid-cols-3 w-full gap-y-5 md:gap-y-10 gap-x-3 md:gap-x-16 items-center">
      {benefitItems.map((data, i) => (
        <div key={i} className="flex flex-col items-center gap-2">
          <h2 className="font-subHeading font-semibold text-white  text-centertext-base lg:text-lg">
            {data.header}
          </h2>
          <p className="text-center text-sm sm:text-base font-medium font-subHeading2 text-gray-300">
            {data.text}
          </p>
        </div>
      ))}
