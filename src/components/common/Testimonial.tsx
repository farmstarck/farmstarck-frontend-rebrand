...
const testimonials = [
  {
    ...
    img: '/assets/images/testimonial-img1.png',
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center justify-center px-0 md:px-16 gap:3 md:gap-10 md:flex-row">
        <Image src={testimonials[current].img} width={300} height={300} alt="" className="w-40 md:w-60" />

        <div className="w-full text-center space-y-2 md:space-y-5 md:text-start md:w-1/2 ">
          <Image width={500} height={500} src={'/assets/svg/quote.svg'} alt="" className="w-5 md:w-10" loading="lazy" />
      ...
      {/* Buttons */}
      <button
        className={`absolute top-1/3 md:top-1/2 left-0 md:left-4 transform translate-y-1/2  p-2 text-gray-400 hover:dark-primary-txt transition ease-out duration-100 md:text-2xl cursor-pointer`}
        onClick={handlePrev}
      >
        ◀
      </button>
      <button
        className={`absolute top-1/3 md:top-1/2 right-0 md:right:4 transform translate-y-1/2 p-2 text-gray-400 hover:dark-primary-txt transition ease-out duration-100 md:text-2xl cursor-pointer`}
        onClick={handleNext}
      >
        ▶
      </button>
