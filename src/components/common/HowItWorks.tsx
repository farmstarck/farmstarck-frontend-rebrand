  ...
  return (
    <div className="flex flex-col items-start relative">
      {steps.map((step, idx) => (
        ...
        >
          {/* Circle Number */}
          <div className="min-w-[40px] h-[40px] flex items-center justify-center rounded-full bg-[var(--primary)] text-white font-bold z-10">
            {step.number}
          </div>

          {/* Line Connector */}
          {idx !== steps.length - 1 && (
            <div className="absolute left-5 top-10 w-0.5 h-full border border-dashed border-[var(--primary)] "></div>
          )}
